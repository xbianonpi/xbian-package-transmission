...
class Transmission(Setting):
    CONTROL = CategoryLabelControl(Tag('label','Transmission'))

class enableTransmission(Setting):
    CONTROL = RadioButtonControl(Tag('label',_('Enable Transmission')))

    def onInit(self):
        self.cfgfile = '/etc/default/transmission-daemon'
        self.setting = 'ENABLE_DAEMON'
        self.exist = False

    def getUserValue(self):
        self.DIALOGHEADER = _('Enable Transmission')
    ...
    def setXbianValue(self,value):
        ...
        if self.exist and value in ('0','1'):
            ...
            return False
        xbianConfig('services', '%s' % ('start' if value == '1' else 'stop'), 'transmission-daemon')
        self.OKTEXT = _('Transmission has been %s now') % (_('started') if value == '1' else _('stopped'))
        return True
